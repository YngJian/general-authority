...
import com.example.authority.domain.entity.*;
...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements IUserService {
    ...
    public IPage<User> findUserDetailList(User user, QueryRequest request) {
        if (StringUtils.isNotBlank(user.getCreateTimeFrom()) &&
            ...
            user.setCreateTimeFrom(user.getCreateTimeFrom() + AuthConstant.DAY_START_PATTERN_SUFFIX);
            user.setCreateTimeTo(user.getCreateTimeTo() + AuthConstant.DAY_END_PATTERN_SUFFIX);
        }
        Page<User> page = new Page<>(request.getPageNum(), request.getPageSize());
        page.setSearchCount(false);
        page.setTotal(baseMapper.countUserDetail(user));
        SortUtil.handlePageSort(request, page, "userId", AuthConstant.ORDER_ASC, false);
    ...
    public void register(String username, String password) {
        ...
        ur.setRoleId(AuthConstant.REGISTER_ROLE_ID);
    ...
    public void updateProfile(User user) {
        ...
        if (isCurrentUser(user.getUserId())) {
            ...
            throw new RuntimeException("您无权修改别人的账号信息！");
        }
    }
